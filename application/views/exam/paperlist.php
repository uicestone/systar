...
$(function(){
	...
	if(tbody.length==0){
		var tbody=$('<tbody></tbody>').appendTo('.contentTable');
	}
	...
	$('#addExamPaper').click(function(){
		...
		.click(function(){
			...
			post.exam=<?=intval($this->uri->segment(3));?>;

			$.post('/exam/listsave?action=exam_paper',post,function(line){
	...
	$('[name="is_scoring"]').change(function(){
		...
		var id=$(this).attr('id');
		$.post('/exam/listsave?update=1',{table:'exam_paper',field:'is_scoring',id:id,value:is_scoring},function(result){
			console.log(result);
