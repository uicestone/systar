<form method="post" name="<?=CONTROLLER?>" id="<?=$this->society->id?>">
	<div class="item">
		...
		<label>教师：</label><?php if($this->value('people/leader')){ ?><?=$this->people->fetch($this->value('people/leader'),'name')?><?php } ?>
		<label>名额：</label><input name="profiles[名额]" value="<?=$this->value('profiles/名额')?>" placeholder="名额" />
		<label>状态：</label>
<?php if($this->user->inTeam('科训')){ ?>
...
<?php } ?>
	...
	<div class="item" name="relative">
		...
		<?=$relative_list?>
<?php if($this->user->id==$this->value('people/leader') || $this->user->isLogged('societyadmin')){ ?>
