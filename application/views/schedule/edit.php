	...
	<div class="contentTable">
		<div class="item">
			<div class="title">
				...
				<? }else{?>
				<button type="button" onclick="javascript:window.opener.parent.focus();window.rootOpener.location.href='/cases/edit/<?=post('schedule/case')?>'">查看</button>
				<? }?>
			</div>
			...
			<? }else{?>
			<div class="field"><?=post('schedule_extra/case_name')?></div>
			<? }?>

			<? if(post('schedule/client')){?>
			<div class="field"><?=post('schedule_extra/client_name')?></div>
			<? }else{?>
		...
		<div class="item">
			...
			<input name="schedule[name]" value="<?=post('schedule/name'); ?>" type="text" />
		</div>

		<div class="item">
			<div class="title"><label>内容：</label></div>
			<textarea class="item" name="schedule[content]" rows="7"><?=post('schedule/content'); ?></textarea>
		</div>
		...
		<div class="item">
			...
			<textarea readonly="readonly" class="item" name="schedule[comment]" rows="5"><?=post('schedule/comment'); ?></textarea>
		</div>
			<? }?>
		<? } ?>
		...
		<div class="item">
			...
			<label>外出地点：<input name="schedule[place]" value="<?=post('schedule/place');?>" type="text" style="width:30%" /></label>
			<label>产生费用：<input name="schedule[fee]" value="<?=post('schedule/fee');?>" type="text" style="width:10%" />元，</label>
		...
		<div class="item">
			...
			<a href="/cases/document<?=post('schedule/document')?>"><?=post('case_document/name')?></a>
			<? }else{ ?>
			...
			<label>备注：</label><input type="text" name="case_document[comment]" value="<?=post('case_document/comment') ?>" style="width:35%" />
		</div>
		<? } ?>
