...
<?php if($this->input->get('project')===false || $this->input->get('project')){ ?>
...
<br />
<?php } ?>
<input type="hidden" name="people" data-placeholder="邀请、通知或关联" class="tagging" multiple="multiple" value="<?=implode(',',$people)?>" data-initselection='<?=json_encode($this->people->getArray(array('id_in'=>$people)))?>' data-ajax="/people/match/"<?php if($this->input->get('people')){ ?> changed="changed"<?php } ?> style="width:98%">
<br />
