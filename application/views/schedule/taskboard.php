...
<? foreach ($column as $task) { ?>
	<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all<?if(!$task['completed']){?> todo<?}?><?if(!$task['completed'] && isset($task['start']) && $task['start']<$this->date->now){?> expired<?}?>" event-id="<?=$task['id']?>">
