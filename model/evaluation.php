<?php
function evaluation_insert_score($indicator,$staff,$field,$value/*,$anonymous*/){
	global $_G;
	...
	if(!db_update('evaluation_score',$data_score,"indicator='".$indicator."' AND staff='".$staff."' AND quarter='".$_G['quarter']."' AND uid='".$_SESSION['id']."' AND company='".$_G['company']."'")){
		return false;
	}
...
function evaluation_getPeer($staff=NULL){
	global $_G;
...
function evaluation_getSelf($staff=NULL){
	global $_G;
...
function evaluation_getManager($staff=NULL){
	global $_G;
